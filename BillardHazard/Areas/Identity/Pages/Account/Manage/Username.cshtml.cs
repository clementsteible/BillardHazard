...
namespace BillardHazard.Areas.Identity.Pages.Account.Manage
{
    public class UsernameModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostChangeEmailAsync()
        {
            ...
            var email = await _userManager.GetEmailAsync(user);
            if (Input.NewUsername != email)
            {
                var userId = await _userManager.GetUserIdAsync(user);
                var code = await _userManager.GenerateChangeEmailTokenAsync(user, Input.NewUsername);
                code = WebEncoders.Base64UrlEncode(Encoding.UTF8.GetBytes(code));
                var callbackUrl = Url.Page(
                    "/Account/ConfirmEmailChange",
                    pageHandler: null,
                    values: new { area = "Identity", userId = userId, email = Input.NewUsername, code = code },
